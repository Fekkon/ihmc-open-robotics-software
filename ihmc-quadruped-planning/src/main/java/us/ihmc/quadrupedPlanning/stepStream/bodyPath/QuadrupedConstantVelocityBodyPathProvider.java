...
//         centerStartPoint.set(startPose.getPosition());
   ...
   private QuadrupedFootstepStatusMessage getLatestStatusMessage(QuadrantDependentList<AtomicReference<QuadrupedFootstepStatusMessage>> footstepStatuses, int depth)
   {
      ...
      sortedMessageList.sort((message1, message2) ->
      {
         if(message1.getDesiredStepInterval().getEndTime() == message2.getDesiredStepInterval().getEndTime())
            return 0;
         else
            return message1.getDesiredStepInterval().getEndTime() > message2.getDesiredStepInterval().getEndTime() ? -1 : 1;
      });
